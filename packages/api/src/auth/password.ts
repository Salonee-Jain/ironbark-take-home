 ...
 * native modules that need a toolchain at install time, and "npm install failed
 * on the reviewer's laptop" is a worse outcome than the marginal difference
 * between two memory-hard KDFs that OWASP both recommends.
 *
 * Parameters are the OWASP minimum for scrypt (N=2^15, r=8, p=1) rather than
 * Node's default N=2^14, and they are stored *in the hash string*. That is the
 * whole point of the encoded format: raising the cost later does not invalidate
 * existing hashes, because each one still says how it was made. A password
 * verified against outdated parameters can be transparently re-hashed on login.
 */
