 ...
 *
 * Driven through `app.inject()` — no port, no socket, no teardown race. The
 * server was built without listening precisely so this would be possible.
 *
 * The suite provisions its own tenant, uploads the real export through
 * `POST /api/uploads`, and asserts what comes back out. That is deliberately
 * the long way round: it exercises signup, the multipart upload, the whole ETL,
 * and every read endpoint against data that arrived the way a client's data
 * actually arrives. It also gives a second tenant to check isolation against,
 * which is the failure that matters most here — a missing `company_id` does not
 * error, it silently reports one company's fuel to another and looks fine.
 *
 * Everything it creates is removed afterwards; the demo company loaded by
 * `npm run etl` is never touched.
 *
...
describe('compliance summary', () => {
   ...
   * model, in a compliance pack. These tests assert the two properties that make
   * it safe rather than the words it happens to contain — every claim resolves
   * to a fact, and nothing survives a check against the current figures that
   * should not.
   */
  /**
   * Availability depends on the environment, not on the code: the committed
   * artefact is served only to a dataset whose facts reproduce the digest it was
   * written against, so a repo whose fact pack has moved since the last
   * generation legitimately has no summary to show. The endpoint contract is
   * asserted unconditionally; the claim-level invariants run against whatever is
   * actually being served, which is the only honest way to check prose that a
   * model wrote.
   */
  ...
  withDb()('drops nothing on read against unchanged data', async () => {
    // Non-zero here means a stored claim no longer matches the figures. It is a
    // legitimate state — the API reports it rather than hiding it — but not one
    // a dataset matching the summary should be in.
    const body = await summaryOrNull();
    if (!body) return;

    expect(body.verification.claimsDroppedOnRead).toBe(0);
    expect(body.verification.factsChanged).toBe(false);
  });
